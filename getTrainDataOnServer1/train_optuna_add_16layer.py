...
def train(num_output_channels):
    ...
    net = Net(num_output_channels,0.30565524503588853,16,64).to(device)
    ...
    optimizer = optim.Adam(net.parameters(), lr=0.0012034275873913057)
    ...
    for epoch in range(num_epochs):
        ...
        if epoch==50:
            torch.save(net, './model/model_layer16_tmp_50_01.pth')
            np.save('./data/lossa/loss_model_layer16_tmp_50_01.npy', np.array([train_loss, test_loss]))
    ...
    # 保存整个模型
    torch.save(net, './model/model_300_optuna_trainloss_326_layer16_03.pth')
...
print(train_loss)
print(test_loss)
np.save('./data/lossa/loss_model_300_optuna_trainloss_326_layer16_03.npy', np.array([train_loss, test_loss]))
