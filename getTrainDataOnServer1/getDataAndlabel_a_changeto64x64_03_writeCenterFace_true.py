...
def calculate_face_centers(matrix_512):
    ...
    # 2*64*65*2改成2*64*65*2，不保存两个面心值矩阵a的最后一列
    return all_vertical_edge_centers[:,:-1], all_horizontal_edge_centers[:,:-1]
...
# 写入标签
with open("./data/label_centerFace_2x64x64x2.txt", "w", encoding="utf-8") as f_write_label:
    ...
    for subdir in subdirs:
        ...
        for content in subdir_contents:
            try:
                ...
                # if float(content) >= 0 and float(content) <= 0.2:#注释地方：1
                    continue
            except ValueError:
                continue
            ...
            with open(file_path, "r") as file:
                ...
                matrix_64, faceCenter_vertical, faceCenter_horizontal = calculate_face_centers(matrix_512)

                f_write_label.write(str(faceCenter_vertical)+"\n\n\n")
                f_write_label.write(str(faceCenter_horizontal)+"\n\n\n")
