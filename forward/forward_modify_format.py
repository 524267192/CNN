...
def printStr(internal_data):
    ...
    def convert_a_to_weights(a_vertical, a_horizontal):
        ...
        three_border = [bottom_border, bottom_border, left_border, left_border]  # 由于边框值上与下相同，左与右相同。顺序是上（左到右），下（左到右），左（下到上），右（下到上）
        three_border = [item.numpy().tolist() for sublist in three_border for item in sublist]  # 存所有边框，四个边框
        all_border = [bottom_border,three_border,[]]
        all_weight = [inner,all_border]
        # print("最终处理后的数据：\n",result)
        return all_weight
